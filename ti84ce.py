#This only works on a TI-84 Plus CE Python Edition

import random

class Field:
    def __init__(self):
        self.bases = [False, False, False]  # 1st, 2nd, 3rd

    def display(self):
        base_names = ['1st', '2nd', '3rd']
        status = []
        for i in range(3):
            if self.bases[i]:
                status.append(base_names[i] + ': Occupied')
            else:
                status.append(base_names[i] + ': Empty')
        print("Bases status:")
        for s in status:
            print(s)
        print("")

    def fill_base(self, base_num):
        if 1 <= base_num <= 3:
            self.bases[base_num - 1] = True

    def empty_base(self, base_num):
        if base_num == 0:
            self.bases = [False, False, False]
        elif 1 <= base_num <= 3:
            self.bases[base_num - 1] = False

class Score:
    def __init__(self):
        self.home_hits = 0
        self.away_hits = 0
        self.home_runs = 0
        self.away_runs = 0

    def update_runs(self, player):
        if player == 1:
            self.home_runs += 1
        else:
            self.away_runs += 1

    def display_score(self, player1_name, player2_name):
        print("|Scoreboard:")
        print("|{} - Runs: {} | Hits: {}".format(player1_name, self.home_runs, self.home_hits))
        print("|{} - Runs: {} | Hits: {}\n".format(player2_name, self.away_runs, self.away_hits))

class Game:
    def __init__(self, player1_name, player2_name, innings):
        self.outs = 0
        self.inning = 1
        self.field = Field()
        self.score = Score()
        self.current_player = 1
        self.bases = [False, False, False]  # First, Second, Third
        self.player1_name = player1_name
        self.player2_name = player2_name
        self.strikes = 0
        self.balls = 0
        self.innings = innings

    def roll(self):
        return sorted([random.randint(1, 6), random.randint(1, 6)])

    def advance_runners(self, bases_to_advance):
        for i in reversed(range(3)):
            if self.bases[i]:
                new_pos = i + bases_to_advance
                if new_pos >= 3:
                    self.bases[i] = False
                    self.score.update_runs(self.current_player)
                    print("{} scored!".format(self.player1_name if self.current_player == 1 else self.player2_name))
                else:
                    self.bases[new_pos] = True
                    self.bases[i] = False

    def handle_hit(self, bases_hit):
        self.advance_runners(bases_hit)
        if bases_hit <= 3:
            self.bases[bases_hit - 1] = True

    def handle_out(self, out_type="Out"):
        self.outs += 1
        print("{}! Outs: {}".format(out_type, self.outs))
        if self.outs >= 3:
            self.end_inning()

    def end_inning(self):
        print("Inning {} over.".format(self.inning))
        self.outs = 0
        self.bases = [False, False, False]
        self.field.empty_base(0)
        self.current_player = 2 if self.current_player == 1 else 1
        if self.current_player == 1:
            self.inning += 1
        self.score.display_score(self.player1_name, self.player2_name)
        self.field.bases = self.bases[:]
        self.field.display()
        if self.inning > self.innings:
            if self.score.home_runs != self.score.away_runs:
                winner = 1 if self.score.home_runs > self.score.away_runs else 2
                print("Game over! {} won after {} innings.".format(
                    self.player1_name if winner == 1 else self.player2_name, self.inning - 1))
                exit()

    def attempt_steal(self):
        if not any(self.bases):
            print("No runners to steal.")
            return

        # Steal from 1st base
        if self.bases[0]:
            print("Attempting to steal from 1st base.")
            steal_roll = random.randint(1, 6)
            if steal_roll in [1, 3, 5]:
                print("Steal from 1st successful!")
                self.bases[1] = True
                self.bases[0] = False
            else:
                print("Steal from 1st failed. Runner out.")
                self.bases[0] = False
                self.handle_out("Steal Out")

        # Steal from 2nd base
        if self.bases[1]:
            print("Attempting to steal from 2nd base.")
            steal_roll = random.randint(1, 6)
            if steal_roll in [2, 5]:
                print("Steal from 2nd successful!")
                self.bases[2] = True
                self.bases[1] = False
            else:
                print("Steal from 2nd failed. Runner out.")
                self.bases[1] = False
                self.handle_out("Steal Out")

        # Steal from 3rd base
        if self.bases[2]:
            print("Attempting to steal from 3rd base.")
            steal_roll = random.randint(1, 6)
            if steal_roll == 4:
                print("Steal from 3rd successful! Player scores!")
                self.score.update_runs(self.current_player)
                self.bases[2] = False
            else:
                print("Steal from 3rd failed. Runner out.")
                self.bases[2] = False
                self.handle_out("Steal Out")

        self.field.bases = self.bases[:]
        self.field.display()

    def update_bases_on_field(self):
        self.field.bases = self.bases[:]
        self.field.display()

    def handle_roll(self):
        r = self.roll()
        print("Dice rolled: {}".format(r))
        # Simplified outcomes for TI-84
        if r == [1, 1] or r == [6, 6]:
            print("Homerun!")
            self.handle_hit(4)
            if self.current_player == 1:
                self.score.home_hits += 1
                self.score.home_runs += 1
            else:
                self.score.away_hits += 1
                self.score.away_runs += 1
            self.bases = [False, False, False]
            self.update_bases_on_field()
            return "Homerun"

        elif r == [1, 2] or r == [2, 1] or r == [5, 5]:
            print("Double!")
            self.handle_hit(2)
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Double"

        elif r == [1, 3] or r == [3, 1]:
            self.handle_out("Fly Out")
            return "Fly Out"

        elif r == [1, 4] or r == [4, 1]:
            print("Walk!")
            # Force runners to advance if needed
            if self.bases[0]:
                if self.bases[1]:
                    if self.bases[2]:
                        self.score.update_runs(self.current_player)
                        print("Runner on third scores from walk!")
                    self.bases[2] = self.bases[1]
                self.bases[1] = self.bases[0]
            self.bases[0] = True
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Walk"

        elif r == [1, 5] or r == [5, 1] or r == [4, 5] or r == [5, 4]:
            self.handle_out("Pop Out")
            return "Pop Out"

        elif r == [1, 6] or r == [6, 1]:
            print("Single!")
            self.handle_hit(1)
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Single"

        elif r == [2, 2]:
            if self.outs < 2 and any(self.bases):
                print("Double Play!")
                self.handle_out("Out 1 (Double Play)")
                self.handle_out("Out 2 (Double Play)")
                return "Double Play"
            else:
                self.handle_out("Groundout")
                return "Groundout"

        elif r == [2, 3] or r == [3, 2]:
            self.handle_out("Groundout")
            return "Groundout"

        elif r == [2, 4] or r == [4, 2]:
            print("Catchers Interference!")
            if self.bases[0]:
                if self.bases[1]:
                    if self.bases[2]:
                        self.score.update_runs(self.current_player)
                        print("Runner on third scores from catchers interference!")
                    self.bases[2] = self.bases[1]
                self.bases[1] = self.bases[0]
            self.bases[0] = True
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Catchers Interference"

        elif r == [3, 4] or r == [4, 3]:
            print("Triple!")
            self.handle_hit(3)
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Triple"

        elif r == [5, 6] or r == [6, 5]:
            self.handle_out("Sacrifice Fly")
            if self.bases[2]:
                self.bases[2] = False
                self.score.update_runs(self.current_player)
                print("Runner scored on Sacrifice Fly!")
            elif self.bases[1]:
                self.bases[2] = True
                self.bases[1] = False
            self.update_bases_on_field()
            return "Sacrifice Fly"

        elif r == [4, 6] or r == [6, 4]:
            if self.bases[0]:
                if random.random() < 0.5:
                    print("Pickoff! Runner on first out.")
                    self.bases[0] = False
                    self.handle_out("Pickoff")
                else:
                    print("Pickoff attempt failed.")
            else:
                print("No runner to pick off.")
            self.update_bases_on_field()
            return "Pickoff"

        elif r == [3, 5] or r == [5, 3]:
            print("Passed Ball!")
            self.advance_runners(1)
            self.update_bases_on_field()
            return "Passed Ball"

        elif r == [3, 6] or r == [6, 3]:
            print("Strike!")
            self.strikes += 1
            if self.strikes == 3:
                print("Strikeout!")
                self.handle_out("Strikeout")
                self.strikes = 0
                self.balls = 0
                return "Strikeout"
            else:
                return "Strike"

        elif r == [2, 5] or r == [5, 2]:
            print("Ball!")
            self.balls += 1
            if self.balls == 4:
                print("Walk on balls")
                if self.bases[0]:
                    if self.bases[1]:
                        if self.bases[2]:
                            self.score.update_runs(self.current_player)
                            print("Runner on third scores from walk!")
                        self.bases[2] = self.bases[1]
                    self.bases[1] = self.bases[0]
                self.bases[0] = True
                self.strikes = 0
                self.balls = 0
                if self.current_player == 1:
                    self.score.home_hits += 1
                else:
                    self.score.away_hits += 1
                self.update_bases_on_field()
                return "Walk"
            else:
                return "Ball"

        elif r == [3, 3] or r == [4, 4]:
            print("Bunt!")
            self.handle_hit(1)
            if self.current_player == 1:
                self.score.home_hits += 1
            else:
                self.score.away_hits += 1
            self.update_bases_on_field()
            return "Bunt"

        elif r == [2, 6] or r == [6, 2]:
            if self.outs == 0 and ((self.bases[0] and self.bases[1]) or (self.bases[1] and self.bases[2])):
                print("TRIPLE PLAY")
                self.outs = 3
            if self.outs < 2 and any(self.bases):
                print("Double Play!")
                self.handle_out("Out 1 (Double Play)")
                self.handle_out("Out 2 (Double Play)")
                return "Double Play"
            else:
                self.handle_out("Groundout")
                return "Groundout"

        else:
            print("Unknown roll.")
            return "Unknown"

    def run_game(self):
        while True:
            print("{}'s turn. Outs: {}, Inning: {}".format(
                self.player1_name if self.current_player == 1 else self.player2_name,
                self.outs, self.inning))
            action = input("Do you want to 'roll' or 'steal'? ").strip().lower()
            if action == "roll":
                result = self.handle_roll()
            elif action == "steal":
                self.attempt_steal()
                result = "Steal attempt"
            else:
                print("Invalid action. Please type 'roll' or 'steal'.")
                continue

            print("Result: {}".format(result))
            self.score.display_score(self.player1_name, self.player2_name)
            self.field.display()

            if self.outs >= 3:
                print("Inning over.")
                self.end_inning()

# Main execution
innings = int(input("How many innings would you like to play? >> "))
player1_name = input("Enter name for Player 1: ")
player2_name = input("Enter name for Player 2: ")

game = Game(player1_name, player2_name, innings)
game.run_game()